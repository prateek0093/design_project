...
const AssignmentCard = ({ title, startTime, endTime, id, data }) => (
    <div className="p-6 bg-white shadow-lg rounded-lg transition-transform duration-200 hover:scale-105">
        ...
        <div className="flex flex-col space-y-1 text-gray-600">
            <p className="flex items-center">
                ...
                <span className="ml-2">{new Date(startTime).toLocaleDateString()}</span>
            </p>
            <p className="flex items-center">
                <span className="font-medium text-gray-700">End Date:</span>{" "}
                <span className="ml-2">{new Date(endTime).toLocaleDateString()}</span>
            </p>
        </div>
